...
export const RBNullNode = new RedBlackTreeNode(null, {color: COLORS.black, __null__: true});
...
export default class RedBlackTree extends BinarySearchTree {
  ...
  min(node = this.root) {
    while (node && node.left !== RBNullNode) {
      node = node.left;
    }
  ...
  max(node = this.root) {
    while (node && node.right !== RBNullNode) {
      node = node.right;
    }
  ...
  successor(node = this.root) {
    if (!node || node === RBNullNode) return null;
    ...
    while (parent !== RBNullNode && node == parent.right) {
      node = parent;
