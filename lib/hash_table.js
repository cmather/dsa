...
export default class HashTable {
  ...
  get(key) {
    let tree = this._table[this.toHashIndex(key)];
    let rbNode = tree.find(new HashTableEntry(key));
  ...
  set(key, value, allowResize = true) {
    ...
    let tree = this._table[ this.toHashIndex(key) ];
    let existingRbNode = tree.find(new HashTableEntry(key));
  ...
  remove(key) {
    ...
    if (tree.remove(new HashTableEntry(key))) {
      this._size--;
      return true;
    }
  ...
  toHashIndex(key) {
    return this._hashFunc(this.toHashKey(key));
  }
